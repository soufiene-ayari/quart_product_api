...
def map_locale(locale: str) -> str:
    """
    Maps incoming locale (like 'en-GB') to internal format (like 'eng_GBR').
    Raises ValueError if unknown locale.
    """
    try:
...
def unmap_locale(locale: str) -> str:
  """
    Reverse of map_locale: given 'ara_ARE' (or 'ara_are'), return 'ar-AE'.
    Raises ValueError if unknown.
  """
  _lang_reverse = {v.lower(): k for k, v in lang_mapping.items()}

  locale=locale.lower()

  if locale in _lang_reverse:
    result = _lang_reverse[locale].lower()
    return result
  else:
    return None
...
def map_brand(brand: str) -> str:
    """
    Maps incoming brand (like 'systemair') to internal format (like 'SYS').
    Raises ValueError if unknown brand.
    """
    try:
...
def get_epimLang_by_market(market: str) -> str:
  m = market.casefold()
  langs: List[str] = []
  seen = set()
  for entry in market_mapping:
    if str(entry.get("Market ID", "")).casefold() == m:
